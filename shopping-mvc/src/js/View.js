...
class View {
  ...
  constructor(model, controller) {
    ...
    /** @param {!HTMLElement} Input list element. */
    ...
    this.quaBox_ = document.getElementById('quantity');
    ...
    this.quaBox_.addEventListener('keyup', (event) => this.onKeyup(event));
  }

  /** Notifies the Controller to add an item to the list. */
  addItem() {
    const trimmedValue = this.inputBox_.value.trim();
    const trimmedQuantity = this.quaBox_.value.trim();
